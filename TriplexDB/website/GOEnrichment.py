...
def go_enrichment(
    ...
    keywords = {
        k: [t.lower() for t in vals] for k, vals in keywords.items()
    }  # We compare lower strings.
    ...
    if not custom_dfs:
        ...
        for cell in dict_keys:
            if len(go_genes[cell]) >= 1:
                ...
                #                      background=None if not background else list(background[cell]))
                query_df = query_df[
                    (~query_df["name"].str.contains("REACTOME"))
                    & (query_df["name"] != "WIKIPATHWAYS")
                    & (~query_df["name"].str.contains("KEGG"))
                    & (~query_df["name"].str.contains("HP root"))
                ...
                query_df["Gene fraction"] = query_df["intersection_size"] / len(
                    go_genes[cell]
                )
    ...
    for source in wanted_sources:
        ...
        for cell in dict_keys:
            if len(term_fetcher[cell][source]) > 0:
                if str(max_terms).lower() == "all":
                    term_collection = term_collection.union(
                        set(term_fetcher[cell][source]["name"].to_list())
                    )
                else:
                    term_collection = term_collection.union(
                        set(term_fetcher[cell][source]["name"].to_list()[:max_terms])
                    )
        ...
        if source in keywords:
            these_terms = [
                x
                for x in these_terms
                if np.any([k.lower() in x.lower() for k in keywords[source]])
            ]
            ...
            for cell in dict_keys:
                ...
                if len(curr_df) > 0:
                    if len(curr_df[curr_df["name"] == term]) == 1:
                        ...
                        pvals.append(
                            next(
                                iter(curr_df[curr_df["name"] == term]["p_value"].values)
                            )
                        )
                    ...
                    if len(match_entry) == 1:
                        main_list.append(
                            [
                                x,
                                y,
                                match_entry["intersection_size"].values[0] / num_genes,
        ...
        if main_list:
            ...
            if len(main_list) > 1 and min(size_col) != max(
                size_col
            ):  # Otherwise all get the same size.
                scaled_sizes = [
                    (
                        (scaled_max - scaled_min)
                        * (s - min(size_col))
                        / (max(size_col) - min(size_col))
                    )
                    + scaled_min
                    for s in size_col
                ]
            ...
            this_height = (
                max([int(len(sorted_terms) * 0.4), 6]) if not fig_height else fig_height
            )
            f, ax = plt.subplots(
                figsize=(fig_width if fig_width else (3 + len(dict_keys)), this_height)
            )
            ...
            if len(go_genes) > 1:
                ...
                plt.scatter(
                    x=[x[0] for x in main_list],
                    y=[y[1] for y in main_list],
                    c=[cmap(norm(c[3])) for c in main_list],
                    s=scaled_sizes,
                    edgecolors=[cmap(norm(c[3])) for c in main_list],
                    zorder=12,
                )

                ax.set_xticks([x for x in range(len(dict_keys))])
                ax.set_xticklabels(
                    [c for c in dict_keys],
                    size=font_s - 6,
                ...
                plt.scatter(
                    x=[abs(np.log2(x[3])) for x in main_list],
                    y=[y[1] for y in main_list],
                    c=["#112791" for c in main_list],  # cmap(norm(c[3]))
                    s=scaled_sizes,
                    edgecolors=["#112791" for c in main_list],
                    zorder=12,
                )
                ...
                ax.set_title(
                    source
                    + ": "
                    + title_tag,  # + (' (#' + str(len(next(iter(go_genes.values())))) + ')') * numerate,
                    size=font_s,
                )
                x_offset = 1.1
            ...
            if len(scaled_sizes) > 2:
                ...
                h2 = Line2D(
                    ...
                    [0],
                    marker="o",
                    markersize=np.sqrt((scaled_min + scaled_max) / 2),
                    color="black",
                    linestyle="None",
                )
                h3 = Line2D(
                ...
                leg = plt.legend(
                    [h1, h2, h3],
                    [
                        str(round(min(size_col), 4)),
                        str(round((min(size_col) + max(size_col)) / 2, 4)),
                        str(round(max(size_col), 4)),
                    ],
                    loc="upper right",
                    markerscale=1,
                    scatterpoints=1,
                    fontsize=font_s - 6,
                    title="Gene fraction",
                    bbox_to_anchor=(legend_out if legend_out else x_offset + 0.5, 1),
                )
            else:
                h3 = Line2D(
                    [0],
            ...
            f.savefig(
                (out_tag + "_" + source + "_max" + str(max_terms) + ".png").replace(
                    " ", ""
                ),
                bbox_inches="tight",
            )
